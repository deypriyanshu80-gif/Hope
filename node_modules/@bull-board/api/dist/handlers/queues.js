...
function pickSetDiagnostics(job) {
...
function getPagination(statuses, counts, currentPage, jobsPerPage) {
    ...
    const total = isLatestStatus
        ? statuses.reduce((total, status) => { var _a; return total + Math.min((_a = counts[status]) !== null && _a !== void 0 ? _a : 0, jobsPerPage); }, 0)
        : ((_a = counts[statuses[0]]) !== null && _a !== void 0 ? _a : 0);
 ...
 */
async function getHasWorkers(queue, showWorkers) {
    if (!showWorkers) {
        return null;
    }
...
async function getAppQueues(pairs, query, showWorkers) {
    return Promise.all(pairs.map(async ([queueName, queue]) => {
        var _a;
        const isActiveQueue = decodeURIComponent((_a = query.activeQueue) !== null && _a !== void 0 ? _a : '') === queueName;
        const jobsPerPage = query.jobsPerPage;
        const jobStatuses = queue.getJobStatuses();
        const status = !isActiveQueue || query.status === 'latest' || !query.status
            ? jobStatuses
            : [query.status];
        const currentPage = query.page;
        const [counts, isPaused, globalConcurrency, activeRateLimitTtl, jobSchedulerCount, hasWorkers,] = await Promise.all([
            queue.getJobCounts(),
            queue.isPaused(),
...
async function queuesHandler(req) {
    ...
    const queues = pairs.length > 0
        ? await getAppQueues(pairs, req.query, ((_a = req.uiConfig) === null || _a === void 0 ? void 0 : _a.showWorkers) !== false)
        : [];
    return {
        body: {
            queues,
