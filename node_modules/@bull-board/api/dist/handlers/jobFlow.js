...
function toFlowNode(node, resolveQueueName) {
    return {
        id: node.job.id,
        name: node.job.name,
...
async function simplifyTree(root, window, resolveQueueName) {
    var _a, _b;
    if (!root || !root.job.id) {
        return null;
    }
...
async function getJobFlow(req, job, queue) {
    ...
    const { buildBoardQueueNameResolver, findFlowRoot, getFlowTree } = await Promise.resolve().then(() => __importStar(require('../providers/flow'))); // required to allow separation between bull & bullMQ
    const root = req.query.root === 'node'
